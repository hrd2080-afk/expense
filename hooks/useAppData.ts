...
export function useAppData() {
  ...
  useEffect(() => {
    if (isLoaded) saveAppState(state);
  }, [state, isLoaded]);
  ...
  const setMonthlyBudget = useCallback(
    (month: string, amount: number, categories?: Record<string, number>) =>
      setState(prev => ({
        ...
        budgets: {
          ...prev.budgets,
          [month]: { month, amount, ...(categories ? { categories } : {}) },
        },
      })),
    [],
  );
  ...
  const addMainCategory = useCallback(
    (name: string, type: MainCategory['type'], color: string) => {
      const newCat: MainCategory = { id: makeMainId(name), name, type, color, subCategories: [] };
      ...
      setState(prev => ({
        ...
        transactions: updates.name
          ? prev.transactions.map(t =>
              t.mainCategoryId === id ? { ...t, mainCategoryName: updates.name! } : t,
            )
          : prev.transactions,
      })),
    [],
  ...
  const addSubCategory = useCallback(
    (mainCategoryId: string, name: string) => {
      const newSub: SubCategory = { id: makeSubId(mainCategoryId, name), name };
      setState(prev => ({
        ...prev,
        categories: prev.categories.map(c =>
          c.id === mainCategoryId ? { ...c, subCategories: [...c.subCategories, newSub] } : c,
        ),
      }));
    },
    [],
